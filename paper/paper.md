...
2. **OIDC Scope Processing:** When the `scope` parameter includes standard OIDC scopes (`openid`, `profile`, `email`, `address`, `phone`), JWTForge auto-populates corresponding claims using Faker.js [@faker], eliminating manual construction of OIDC-conformant claim sets [@openid_connect].
...
- *Fuzz mode*: One to three claims (excluding protected `iss`, `jti`, and caller-excluded fields) are replaced with entries drawn uniformly at random from the BLNS corpus [@blns] (507 strings) augmented with edge cases (`null`, `undefined`, `Infinity`, `NaN`, deeply nested objects, large arrays, numeric boundaries).
